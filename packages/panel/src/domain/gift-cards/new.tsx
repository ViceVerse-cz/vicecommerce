...
const NewGiftCard = ({ onClose }: NewGiftCardProps) => {
  ...
  const { register, setValue, handleSubmit, control } = useForm<NewGiftCardFormData>({
  ...
  const onSubmit = async (data: NewGiftCardFormData) => {
    ...
    if (thumbnail) {
      const uploadedImgs = await Medusa.uploads.create([thumbnail.nativeFile]).then(({ data }) => {
    ...
    mutate(
      {
        ...
        variants: data.denominations.map((d, i) => ({
          ...
          prices: [{ amount: d.amount!, currency_code: store?.default_currency_code }],
