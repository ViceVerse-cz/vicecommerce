...
export const formValuesToUpdateGiftCardMapper = async (
  ...
  const imagePayload = {} as Pick<AdminPostProductsProductReq, "images" | "thumbnail">;
  ...
  if (values.thumbnail) {
    imagePayload.thumbnail = imagePayload.images?.length ? imagePayload.images[values.thumbnail] : undefined;
  }
  ...
  return {
    ...
    type: values.type ? { id: values.type?.value, value: values.type.label } : undefined,
...
export const giftCardToFormValuesMapper = (giftCard: Product): ManageGiftCardFormData => {
  let thumbnail = giftCard.images?.length ? giftCard.images.findIndex((img) => img.url === giftCard.thumbnail) : 0;
  ...
  return {
    ...
    type: giftCard.type ? { label: giftCard.type.value, value: giftCard.type.id } : null,
