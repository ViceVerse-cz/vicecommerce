...
const AddAdditionalItemsScreen = ({ append, remove, selectedIds, order }: Props) => {
  ...
  const numPages = useMemo(() => (count ? Math.ceil(count / PAGE_SIZE) : 0), [count]);
  ...
  const isRowSelectable = (row: Row<PricedVariant>) => {
    ...
    const hasRegionPrice = row.original.prices.some((p) => p.region_id === region_id);
    const hasCurrencyPrice = row.original.prices.some((p) => p.currency_code === currency_code);
  ...
  const onSubmit = () => {
    const selectedVariants = getSelectedRowModel().rows.map((row) => row.original);
  ...
  return (
    <>
      <Modal.Content>
        <AddAdditionalItemsTable instance={table} isLoadingData={isLoading} setSearchTerm={setQuery} />
      ...
      <Modal.Footer>
        <div className="flex w-full items-center justify-end gap-x-xsmall">
          ...
          <Button variant="primary" size="small" type="button" onClick={onSubmit}>
            Add products
          </Button>
        </div>
      </Modal.Footer>
