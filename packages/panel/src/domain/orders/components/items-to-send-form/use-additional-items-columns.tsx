...
export const useAdditionalItemsColumns = ({ form, orderCurrency, removeItem }: AdditionalItemsColumnProps) => {
  ...
  const columns = useMemo(() => {
    return [
      columnHelper.display({
        id: "product_display",
        header: "Product",
        cell: ({ row: { original: { thumbnail, product_title, variant_title, sku } } }) => {
          return (
            <div className="flex items-center gap-base">
              <Thumbnail src={thumbnail} />
              <div>
                <p>
                  {product_title} <span className="text-grey-50">({variant_title})</span>
                </p>
      ...
      columnHelper.display({
        ...
        maxSize: 50,
        cell: ({ row: { index, original: { in_stock } } }) => {
          return (
      ...
      columnHelper.accessor("price", {
        ...
        cell: ({ getValue, row: { original: { original_price } } }) => {
      ...
      columnHelper.display({
        ...
        maxSize: 20,
        cell: () => {
          return <p className="pl-base text-grey-50">{orderCurrency.toUpperCase()}</p>;
        },
      }),
      columnHelper.display({
        id: "remove",
