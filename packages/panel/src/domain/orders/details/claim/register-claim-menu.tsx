...
import ClaimTypeForm, { ClaimTypeFormType } from "../../components/claim-type-form";
import ItemsToReturnForm, { ItemsToReturnFormType } from "../../components/items-to-return-form";
import ItemsToSendForm, { ItemsToSendFormType } from "../../components/items-to-send-form";
import { RefundAmountFormType } from "../../components/refund-amount-form";
import { ClaimSummary } from "../../components/rma-summaries";
import SendNotificationForm, { SendNotificationFormType } from "../../components/send-notification-form";
...
const RegisterClaimMenu = ({ order, onClose }: Props) => {
    ...
    mutate(
      {
        ...
        refund_amount: type === "refund" && refundAmount !== undefined ? refundAmount : undefined,
        shipping_address:
          type === "replace"
            ? {
  ...
  return (
    <LayeredModal open={true} handleClose={onCancel} context={context} isLargeModal>
      <Modal.Body>
        ...
        <form onSubmit={onSubmit} data-testid="register-claim-form">
          <Modal.Content>
            <div className="flex flex-col gap-y-xlarge">
              <ItemsToReturnForm form={nestedForm(form, "return_items")} order={order} isClaim={true} />
              <ShippingForm form={nestedForm(form, "return_shipping")} order={order} isReturn={true} isClaim={true} />
              <ClaimTypeForm form={nestedForm(form, "claim_type")} />
              {watchedType === "replace" && (
                <>
                  <ItemsToSendForm form={nestedForm(form, "additional_items")} order={order} />
                  <ShippingAddressForm form={nestedForm(form, "shipping_address")} order={order} />
                  <ShippingForm form={nestedForm(form, "replacement_shipping")} isClaim={true} order={order} />
                </>
              )}
          ...
          <Modal.Footer>
            <div className="flex w-full items-center justify-between">
              <SendNotificationForm form={nestedForm(form, "notification")} type="claim" />
              <div className="flex items-center justify-end gap-x-xsmall">
                <Button variant="secondary" size="small" type="button" onClick={onCancel}>
                  Cancel
                </Button>
                <Button variant="primary" size="small" disabled={!isDirty || isLoading || watchedItems?.length < 1}>
                  Submit and close
                </Button>
              </div>
            </div>
