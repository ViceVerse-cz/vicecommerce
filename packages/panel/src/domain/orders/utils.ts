 ...
 */
export function transformFiltersAsExportContext(filters: Record<string, { filter: string[] }>) {
  const context = {
    filterable_fields: {
  ...
  for (const k in context.filterable_fields) {
    if (context.filterable_fields[k] === null || context.filterable_fields[k]?.length === 0) {
      context.filterable_fields[k] = undefined;
    }
  }
