import { AdminPostDiscountsDiscountReq, AdminPostDiscountsReq, AdminUpsertCondition, Discount } from "@medusajs/medusa";
...
export const discountToFormValuesMapper = (discount: Discount): DiscountFormValues => {
  return {
    code: discount.code,
    rule: {
...
export const formValuesToCreateDiscountMapper = (
  ...
  return {
    code: values.code!,
    rule: {
      allocation: values.rule.type === DiscountRuleType.FIXED ? values.rule.allocation : AllocationType.TOTAL,
    ...
    valid_duration: values.is_dynamic && values.valid_duration?.length ? values.valid_duration : undefined,
  };
};
...
export const formValuesToUpdateDiscountMapper = (
  ...
  return {
    code: values.code,
    rule: {
      allocation: values.rule.type === "fixed" ? AllocationType.ITEM : AllocationType.TOTAL,
      id: ruleId,
    ...
    valid_duration: values.valid_duration?.length ? values.valid_duration : undefined,
  };
};
