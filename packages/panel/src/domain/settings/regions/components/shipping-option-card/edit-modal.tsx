...
import ShippingOptionForm, { ShippingOptionFormType } from "../shipping-option-form";
      ...
      <Modal.Body>
        ...
        <form onSubmit={onSubmit}>
          <Modal.Content>
            ...
            <ShippingOptionForm form={form} region={option.region} isEdit={true} />
          </Modal.Content>
          <Modal.Footer>
            <div className='flex items-center gap-x-xsmall justify-end w-full'>
              ...
              <Button variant='primary' size='small' type='submit' loading={isLoading} disabled={isLoading || !isDirty}>
                Uložit a zavřít
              </Button>
            </div>
          </Modal.Footer>
...
const getDefaultValues = (option: ShippingOption): ShippingOptionFormType => {
  ...
  return {
    ...
    name: option.name,
    price_type: {
      label: option.price_type === "flat_rate" ? "Flat rate" : "Calculated Price",
