          ...
          <div className='flex flex-col gap-y-xsmall'>
            <RegionDetail title={"Currency"}>
              <div className='flex items-center gap-x-xsmall'>
                <span className='inter-base-semibold text-grey-90'>{region.currency_code.toUpperCase()}</span>
                        ...
                        content={
                          <ul>
                            {region.payment_providers.slice(4).map((p) => {
                              return <li key={p.id}>{paymentProvidersMapper(p.id)}</li>;
                            })}
                          </ul>
                        }
                      >
                        ...
                        content={
                          <ul>
                            {region.fulfillment_providers.slice(4).map((p) => {
                              return <li key={p.id}>{fulfillmentProvidersMapper(p.id)}</li>;
                            })}
                          </ul>
                        }
                      >
                        <span className='cursor-default'>+ {region.fulfillment_providers.length - 4} více</span>
                      </Tooltip>
                    )}
                  </div>
                ) : (
