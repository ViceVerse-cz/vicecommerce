...
const CurrentCurrenciesScreen = () => {
  ...
  const onRemove = () => {
    const currencies = store.currencies.filter((sc) => !selectedRowIds.includes(sc.code)).map((c) => c.code);
    onSubmit(currencies);
    onDeselect();
  };
    ...
    <>
      ...
      <Modal.Content>
        <CurrenciesTable
          ...
          offset={offset}
          tableAction={
            <TableActions numberOfSelectedRows={selectedRowIds.length} onDeselect={onDeselect} onRemove={onRemove} />
          }
        />
      </Modal.Content>
      <Modal.Footer>
...
const TableActions = ({ numberOfSelectedRows, onDeselect, onRemove }: TableActionProps) => {
