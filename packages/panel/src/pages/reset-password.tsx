...
const ResetPasswordPage = () => {
  ...
  useEffect(() => {
    if (formState.dirtyFields.password && formState.dirtyFields.repeat_password) {
      setReady(true);
    } else {
      setReady(false);
    }
          ...
          >
            ...
            {!token ? (
              <div className='h-full flex flex-col gap-y-2 text-center items-center justify-center'>
                <span className='inter-large-semibold text-grey-90'>You reset link is invalid</span>
                <span className='inter-base-regular text-grey-50 mt-2'>Please try resetting your password again</span>
              </div>
            ) : (
              <>
                <span className='inter-2xlarge-semibold mt-4 text-grey-90'>Reset your password</span>
                <span className='inter-base-regular text-grey-50 mt-2 mb-xlarge'>Choose a new password below 👇🏼</span>
                <SigninInput placeholder='Email' name='first_name' value={email} readOnly />
                <SigninInput
