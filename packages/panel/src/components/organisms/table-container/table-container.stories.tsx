...
const columns: Column<TestObject>[] = [
  ...
  },
  {
    Header: "Date",
    accessor: "date",
    Cell: ({ value }) => <div className='text-grey-50'>{value.toLocaleDateString()}</div>,
  },
  {
    Header: "Result",
    accessor: "result",
  ...
  return (
    <div>
      ...
      <div className='flex items-start justify-between mt-xlarge'>
        <Button className='mt-base' variant='secondary' size='small' onClick={() => setOffset(random(0, 10))}>
          Reload
        </Button>
        <p className='inter-small-regular text-grey-50 inline-block max-w-[440px]'>
          Note that React Table does not work in storybook. Because of this pagination does not work in this example,
          you should instead use the reload button to simulate pagination.
        </p>
      </div>
    </div>
  );
