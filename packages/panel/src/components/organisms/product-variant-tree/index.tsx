...
const ProductVariantTree: React.FC<ProductVariantTreeProps> = ({ product, getProductActions, getVariantActions }) => {
  return (
    <CollapsibleTree>
      <CollapsibleTree.Parent actions={getProductActions?.(product as Product)}>
        <div>
      ...
      <CollapsibleTree.Content>
        {product.variants.map((variant) => (
          <CollapsibleTree.Leaf key={variant.id} actions={getVariantActions?.(variant as ProductVariant)}>
      ...
      <div className='flex items-center text-grey-50 flex-1 justify-end'>
        <div className='text-grey-50 mr-xsmall'>
          {filteredPrices.length ? (
            <span>{`${filteredPrices.length} price${filteredPrices.length > 1 ? "s" : ""}`}</span>
          ) : (
