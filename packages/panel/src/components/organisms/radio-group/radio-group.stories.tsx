...
const values = [
  {
    id: "1",
    name: "Americas",
    countries: [{ display_name: "United States" }, { display_name: "Canada" }, { display_name: "Mexico" }],
      ...
      {values.map((value) => {
        return (
          <RadioGroup.Item
            ...
            value={value.id}
            label={value.name}
            sublabel={`(${value.countries.map((c) => c.display_name).join(", ")})`}
...
const Template = (args) => (
  <div>
    <RadioGroup.Root {...args}>
      {values.map((value) => {
        return <RadioGroup.Item key={value.id} value={value.id} label={value.name} description={value.description} />;
      })}
    </RadioGroup.Root>
  </div>
);
