...
function FileSummary(props: FileSummaryProps) {
  ...
  const formattedSize =
    size / 1024 < 10 ? `${(size / 1024).toFixed(2)} KiB` : `${(size / (1024 * 1024)).toFixed(2)} MiB`;
    ...
    <div className='relative'>
      ...
      <div className='relative flex items-center rounded-xl border border-1 mt-6'>
        ...
        <div className='flex-1 my-6'>
          ...
          <div className='text-xsmall leading-4 text-grey-50'>{status || formattedSize}</div>
        </div>
    ...
    <Modal open handleClose={onClose}>
      <Modal.Body>
        <Modal.Content>
          ...
          {summary && <UploadSummary creations={summary.toCreate} updates={summary.toUpdate} type={type} />}
          ...
          <div className='text-grey-90 text-base inter-large-semibold mt-8'>{description2Title}</div>
          ...
          <FileSummary
            ...
            size={2967}
            action={
              <a className='w-6 h-6 cursor-pointer' href={templateLink} download>
