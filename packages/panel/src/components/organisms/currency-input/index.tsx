...
const Root: React.FC<CurrencyInputProps> = ({
  ...
  const [selectedCurrency, setSelectedCurrency] = useState<CurrencyType | undefined>(getCurrencyInfo(currentCurrency));
        ...
        {!hideCurrency && (
          ...
          >
            {!readOnly ? (
              ...
              <Input label='Currency' value={value?.label} readOnly className='pointer-events-none' tabIndex={-1} />
            )}
          </div>
        )}
  ...
  (
    ...
    const handleChange = (value?: string, floatValue?: number | null) => {
      ...
      if (typeof floatValue === "number" && currencyInfo) {
        persistedAmount = Math.round(persistedPrice(currencyInfo.code, floatValue));
      } else {
      ...
      <div {...rest}>
        ...
        >
          {currencyInfo?.symbol_native && (
            <Tooltip open={invalid} side={"top"} content={invalidMessage || "Amount is not valid"}>
              <span className='inter-base-regular text-grey-40 mr-xsmall'>{currencyInfo.symbol_native}</span>
            </Tooltip>
          )}
          <AmountField
            ...
            onValueChange={(value, _name, values) => handleChange(value, values?.float)}
            ref={inputRef}
            step={step}
