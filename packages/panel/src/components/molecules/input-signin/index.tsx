...
import React, { ChangeEventHandler, FocusEventHandler, useEffect, useImperativeHandle, useRef, useState } from "react";
...
const SigninInput = React.forwardRef(
  ({ placeholder, name, key, required, onChange, onFocus, className, type, ...props }: InputProps, ref) => {
    ...
    return (
      ...
      >
        {props.readOnly && <LockIcon size={16} className='text-grey-40 ml-base' />}
        <input
          className={clsx("outline-none outline-0 remove-number-spinner leading-base bg-transparent w-full py-3 px-4", {
