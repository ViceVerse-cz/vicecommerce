...
const TextArea = React.forwardRef<HTMLTextAreaElement, TextareaProps>(
  (
    ...
    const handleAddEmoji = (emoji: string) => {
      ...
      const newValue = `${inputRef.current?.value.substring(0, position)}${emoji}${inputRef.current?.value.substring(
        position,
      )}`;
    ...
    return (
      <div className={className} {...containerProps}>
        {label && (
          <InputHeader {...{ label, required, withTooltip, tooltipText, tooltipProps }} className='mb-xsmall' />
        )}
        <div
          className={clsx(
