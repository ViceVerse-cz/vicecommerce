...
const Description: React.FC<BannerCardDescriptionProps> = ({ cta, children }) => {
  return (
    <div>
      <p className='inter-small-regular text-grey-50 max-w-[460px] line-clamp-2'>{children}</p>
      {cta && (
        <button className='text-violet-60 inter-small-semibold mt-base' onClick={cta.onClick}>
          {cta.label}
        </button>
      )}
    </div>
