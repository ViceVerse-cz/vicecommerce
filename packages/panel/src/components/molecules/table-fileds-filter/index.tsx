...
function Chip(props: ChipProps) {
  ...
  return (
    <div className='rounded-lg h-[32px] inline-flex gap-1 shrink-0 items-center text-small text-grey-70 border border-gray-70 px-3 mr-1 last:mr-2'>
      {short}
      <CrossIcon className='text-grey-40 cursor-pointer' onClick={remove} size={13} />
    </div>
  );
}
  ...
  return (
    <DropdownMenu.Item>
      <Checkbox
        ...
        onChange={onChange}
        label={typeof field.label === "function" ? field.label({ isSelected: props.checked }) : field.label}
      />
    </DropdownMenu.Item>
  );
}
...
function TableFieldsFilters(props: TableFieldsFilterProps) {
  ...
  return (
    <div className='flex-wrap flex items-center gap-y-2'>
      ...
      <FieldsMenu fields={sortBy(fields, "id")} onBlur={setSelectedFields} selectedFields={_selected} />
    </div>
  );
}
