...
const EditCreated: React.FC<EditCreatedProps> = ({ event }) => {
  const { isModalVisible, showModal, setActiveOrderEdit } = useContext(OrderEditContext);
...
const OrderEditChangeItem: React.FC<OrderEditChangeItemProps> = ({ change }) => {
  let quantity;
  ...
  if (isAdd) {
    ...
    quantity = (change.original_line_item as LineItem)?.quantity - (change.line_item as LineItem)?.quantity;
  }
    ...
    <div className='flex gap-x-base mt-xsmall'>
      ...
      <div className='flex flex-col'>
        <span className='inter-small-semibold text-grey-90'>
          ...
          {lineItem?.variant?.sku && <CopyToClipboard value={lineItem?.variant?.sku} iconSize={14} />}
        </span>
