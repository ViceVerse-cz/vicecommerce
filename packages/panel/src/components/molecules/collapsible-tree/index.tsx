...
const CollapsibleTreeContext = React.createContext<TCollapsibleTreeContext | null>(null);
...
const CollapsibleTreeContent: React.FC<CollapsibleTreeContentProps> = ({ children, className, ...props }) => {
...
const CollapsibleTreeParent: React.FC<CollapsibleTreeParentProps> = ({ actions, className, children }) => {
      ...
      <Container className={className}>
        <div className='flex items-center justify-between'>
          ...
          <div className='flex items-center gap-x-xsmall'>
            ...
            <Button variant='ghost' size='small' className='p-[6px] text-grey-50' onClick={toggle}>
...
const CollapsibleTreeLeaf: React.FC<CollapsibleTreeLeafProps> = ({ className, children, actions }) => {
  return (
    <div className={clsx("col-tree flex items-center relative pb-xsmall last:pb-0", className)}>
      ...
      <Container className='w-full flex items-center justify-between inter-small-regular'>
        {children}
        {actions && <Actionables forceDropdown customTrigger={Trigger()} actions={actions} />}
      </Container>
    </div>
  );
};
...
const Container: React.FC<{ className?: string }> = ({ children, className }) => {
  return <div className={clsx("rounded-rounded border border-grey-20 py-2xsmall px-small", className)}>{children}</div>;
};
