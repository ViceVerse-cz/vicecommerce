...
export const ActivityCard: React.FC<ActivityCardProps> = (props: ActivityCardProps) => {
  const { key, title, icon, relativeTimeElapsed, shouldShowStatus, children } = props;
  ...
  const getTimeElement = () => {
    return (
      <div className='flex cursor-default'>
        ...
        {shouldShowStatus && <StatusIndicator variant={"primary"} className='ml-2' />}
      </div>
    );
  };
        ...
        <div className='relative w-full h-full'>
          <div className='flex justify-between inter-small-semibold text-grey-90'>
            ...
            {(!!relativeTimeElapsed || shouldShowStatus) &&
              (formattedDate ? <Tooltip content={formattedDate}>{getTimeElement()}</Tooltip> : getTimeElement())}
          </div>
