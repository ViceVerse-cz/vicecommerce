...
const FilterDropdownItem = ({
  ...
  const onCheck = (filter) => {
    ...
    const newFilter = Object.entries(checkedState).reduce((acc, [key, value]) => {
      if (value === true) {
        acc.push(key);
      }
      return acc;
    ...
    >
      ...
      >
        <RadixCollapsible.Trigger
          className={clsx("py-1.5 px-3 flex w-full items-center hover:bg-grey-5 rounded justify-between", {
            "inter-small-semibold": open,
            "inter-small-regular": !open,
          })}
        >
          <div className='flex items-center'>
            ...
            <input id={filterTitle} className='hidden' checked={open} readOnly type='checkbox' />
        ...
        <RadixCollapsible.Content className='w-full'>
          ...
          {isLoading ? (
            ...
            options.map((el, i) => {
              ...
              return (
                <div
                  className={clsx("w-full flex hover:bg-grey-20 my-1 py-1.5 pl-6 items-center rounded", {
                ...
                >
                  ...
                  >
                    <span className='self-center'>{checked[value] === true && <CheckIcon size={16} />}</span>
                  </div>
                  <input
                    type='checkbox'
                    className='hidden'
...
const DateFilter = ({ options, open, setFilter, existingDate, existingFilter }) => {
  ...
  const [relativeAmount, setRelativeAmount] = useState(initialVals.relativeAmount);
  const [daysMonthsValue, setDaysMonthsValue] = useState(initialVals.daysMonthsValue);
    ...
    switch (currentFilter) {
      ...
      case DateFilters.EqualTo: {
        ...
        if (momentToSet) {
          ...
          const nextDay = dateToUnixTimestamp(addHours(momentToSet, 24).toDate());
        ...
        return (
          <div className='flex flex-col w-full'>
            <RightPopover
              trigger={
                <div className='flex w-full items-center justify-between bg-grey-5 border border-grey-20 rounded inter-small-semibold text-grey-90 px-3 py-1.5'>
                  <label>{startDate ? moment(startDate).format("MM.DD.YYYY") : "-"}</label>
                  <span className='text-grey-50'>
                    <ArrowRightIcon size={16} />
                  </span>
                </div>
  ...
  return (
    <div className='pl-9'>
      ...
      >
        <PopoverOptions options={options} onClick={(filter) => setCurrentFilter(filter)} selectedItem={currentFilter} />
      </RightPopover>
    ...
    <>
      {options.map((item) => (
        <div
          ...
          className={clsx("px-3 py-1.5 my-1 flex items-center rounded hover:bg-grey-5 cursor-pointer", {
            "inter-small-semibold": item === selectedItem,
            "inter-small-regular": item !== selectedItem,
          })}
        >
          <div
            className={clsx("rounded-full flex items-center justify-center mr-2 w-4 h-4", {
...
const RightPopover = ({ trigger, children }) => (
  <RadixPopover.Root>
    <RadixPopover.Trigger className='w-full my-1'>{trigger}</RadixPopover.Trigger>
