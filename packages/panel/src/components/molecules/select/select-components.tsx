  ...
  return (
    <components.Menu
      className={clsx("!rounded-rounded", {
        "-mt-1 z-60": !props.selectProps.isSearchable && props.menuPlacement === "bottom",
        "mb-3 z-60": !props.selectProps.isSearchable && props.menuPlacement === "top",
      })}
      {...props}
    >
      {props.children}
...
const Placeholder = <Option, IsMulti extends boolean, Group extends GroupBase<Option>>(
  ...
  return props.selectProps.menuIsOpen ? null : <components.Placeholder {...props} className='!mx-0 !text-grey-40' />;
};
...
const ClearIndicator = <Option, IsMulti extends boolean, Group extends GroupBase<Option>>({
  ...
  return (
    <div {...innerProps} className='hover:bg-grey-10 text-grey-50 rounded cursor-pointer'>
      <XCircleIcon size={16} />
    </div>
  );
};
...
const CheckboxAdornment = ({ isSelected }: { isSelected: boolean }) => {
  return (
    <div
      className={clsx("w-5 h-5 flex justify-center text-grey-0 border-grey-30 border rounded-base", {
...
const RadioAdornment = ({ isSelected }: { isSelected: boolean }) => {
  return (
    <div
      className={clsx("radio-outer-ring outline-0", "shrink-0 w-[20px] h-[20px] rounded-circle", {
...
const NoOptionsMessage = <Option, IsMulti extends boolean, Group extends GroupBase<Option>>({
  ...
  return (
    <div className='text-grey-50 inter-small-semibold text-center p-xsmall' {...innerProps}>
...
const Option = <Option, IsMulti extends boolean, Group extends GroupBase<Option>>({
  ...
  return (
    <components.Option {...props} className='my-1 py-0 px-2 bg-grey-0 active:bg-grey-0'>
        ...
        <div className='items-center h-full flex'>
          {props.data?.value !== "all" && props.data?.label !== "Select All" ? (
            <>
              ...
              <span className='ml-3 text-grey-90 inter-base-regular'>{props.data.label}</span>
            </>
          ) : (
