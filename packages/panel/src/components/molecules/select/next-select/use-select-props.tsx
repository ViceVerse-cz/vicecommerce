...
>({
  ...
  const sortOptions = (values: Option[]) => {
    ...
    const unselectedOptions = stateOptions.filter((option) => !tmp.find((op) => isEqual(op, option)));
  ...
  const onChange = (newValue: OnChangeValue<Option, IsMulti>, actionMeta: ActionMeta<Option>) => {
    if (isMulti) {
