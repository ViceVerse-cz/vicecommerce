...
export const optionIsDisabled = (option: unknown): option is { isDisabled: boolean } =>
...
export const isCreateOption = (option: unknown): option is { __isNew__: true } => {
  return typeof option === "object" && option !== null && "__isNew__" in option;
};

export const formatOptionLabel = <Option,>(option: Option, { inputValue }: FormatOptionLabelMeta<Option>) => {
  if (!hasLabel(option)) {
    return;
  }
  ...
  return <Highlighter searchWords={[inputValue]} textToHighlight={option.label} highlightClassName='bg-orange-10' />;
};
