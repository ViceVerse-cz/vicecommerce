...
const SSelect = React.forwardRef(
  (
    ...
    const onClickOption = (val, ...args) => {
      if (val?.length && val?.find((option) => option.value === "all") && hasSelectAll && isMultiSelect) {
        onChange(options);
      } else {
        onChange(val);
        if (!isMultiSelect) {
        ...
        >
...

          {
            <GetSelect
              ...
              options={hasSelectAll && isMultiSelect ? [{ value: "all", label: "Select All" }, ...options] : options}
              ref={selectRef}
              value={value}
              ...
              closeMenuOnScroll={(e) => {
                if (!scrollBlocked && e.target?.contains(containerRef.current) && e.target !== document) {
                  return true;
                }
              }}
...
const GetSelect = React.forwardRef(({ isCreatable, searchBackend, onCreateOption, handleClose, ...props }, ref) => {
  if (isCreatable) {
    return searchBackend ? (
      ...
      <CreatablePrimitive {...props} isSearchable ref={ref} onCreateOption={onCreateOption} />
    );
  } else if (searchBackend) {
    return <AsyncPrimitive ref={ref} defaultOptions={true} loadOptions={searchBackend} {...props} />;
  }
