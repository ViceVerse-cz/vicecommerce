...
const SteppedModal: React.FC<SteppedProps> = ({
  ...
  return (
    <ModalElement layeredContext={layeredContext} isLargeModal={isLargeModal} handleClose={resetAndClose}>
      <Modal.Body className={clsx("transition-transform flex flex-col justify-between duration-100 max-h-full")}>
        <Modal.Header handleClose={resetAndClose}>
          <div className='flex flex-col'>
            <h2 className='inter-xlarge-semibold'>{title}</h2>
            {!lastScreenIsSummary ||
              (lastScreenIsSummary && context.currentStep !== steps.length - 1 && (
      ...
      <Modal.Footer>
        <div className='flex justify-end w-full gap-x-xsmall'>
          ...
          <Button
            ...
            onClick={() => (context.currentStep === steps.length - 1 ? resetAndSubmit() : context.goToNextPage())}
            className='w-[112px]'
          >
...
const ModalElement = ({ layeredContext, handleClose, isLargeModal = true, children }) =>
  layeredContext ? (
    <LayeredModal context={layeredContext} handleClose={handleClose} isLargeModal={isLargeModal}>
      {children}
    </LayeredModal>
  ) : (
