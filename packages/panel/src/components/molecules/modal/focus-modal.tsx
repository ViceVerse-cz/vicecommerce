...
const FocusModal: IFocusModal = ({ className, children }) => (
  <div className={clsx("absolute inset-0 bg-grey-0 z-50 flex flex-col items-center", className)}>{children}</div>
);

FocusModal.Header = ({ children, className }) => (
  <div className={clsx("w-full border-b py-4 border-b-grey-20 flex justify-center", className)}>{children}</div>
);
...
const BasicFocusModalHeader: React.FC<BasicFocusModalProps> = ({ handleClose, onSubmit, cancelText, submitText }) => {
  return (
    <FocusModal.Header>
