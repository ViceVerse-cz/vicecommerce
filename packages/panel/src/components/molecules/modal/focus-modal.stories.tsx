...
const BasicTemplate: ComponentStory<typeof FocusModal> = (args) => {
  ...
  return (
    <div>
      {open && (
        <FocusModal.BasicFocusModal onSubmit={() => setOpen(false)} handleClose={() => setOpen(false)}>
          {args.children}
...
const Block = ({ children }) => <div className='w-full h-[100px] my-4 bg-grey-10 rounded-rounded'>{children}</div>;
