...
const Checkbox = React.forwardRef(({ label, value, className, id, ...rest }: CheckboxProps, ref) => {
  const checkboxRef = React.useRef<HTMLInputElement>(null);

  useImperativeHandle(ref, () => checkboxRef.current);
  return (
    <label className={clsx("flex items-center cursor-pointer", className)} htmlFor={id}>
      <input
        type='checkbox'
        ref={checkboxRef}
