...
const ProductOverview = ({ products, toggleListView }: ProductOverviewProps) => {
  return (
    <>
      <div className='flex justify-end border-t border-b border-grey-20 py-2.5 pr-xlarge'>
        <div className='inter-small-semibold text-grey-50 flex justify-self-end'>
          <span onClick={toggleListView} className={clsx("hover:bg-grey-5 cursor-pointer rounded p-0.5 text-grey-40")}>
            <ListIcon size={20} />
          </span>
          <span className={clsx("hover:bg-grey-5 cursor-pointer rounded p-0.5 text-grey-90")}>
            <TileIcon size={20} />
          </span>
        </div>
      </div>
  ...
  return (
    <div className='p-base group rounded-rounded hover:bg-grey-5 flex-col'>
      <div className='relative'>
        <div className={clsx("rounded-base inline-block absolute top-2 right-2")}>
          <Actionables
            actions={getActions()}
            customTrigger={
              <Button
        ...
        <Link to={`${product.id}`}>
          ...
          <div>
            <div className='mt-base flex items-center justify-between'>
              <p className='inter-small-regular text-grey-90 line-clamp-1 mr-3'>{product.title}</p>
              <StatusIndicator variant={getProductStatusVariant(product.status)} className='shrink-0' />
            </div>
            <span className='inter-small-regular text-grey-50 line-clamp-1'>{product.collection?.title}</span>
          </div>
        </Link>
      </div>
    </div>
