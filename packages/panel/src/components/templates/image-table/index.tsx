...
const ImageTable = ({ data, form, onDelete }: ImageTableProps) => {
  ...
  const columns = useMemo<Column<{ id?: string | undefined } & FormImage>[]>(() => {
    return [
      {
        Header: () => (
        ...
        Cell: ({ cell: { value } }) => {
          return (
            <div className='py-base ml-large'>
              <img className='h-[80px] w-[80px] object-cover rounded' src={value} />
            </div>
          );
        },
      },
              ...
              render={({ field: { value } }) => {
                return (
                  <div className='w-full'>
                    ...
                    {value?.size && (
                      <span className='inter-small-regular text-grey-50'>{(value.size / 1024).toFixed(2)} KB</span>
                    )}
                  </div>
                );
              }}
        ...
        {headerGroups?.map((headerGroup) => (
          <Table.HeadRow {...headerGroup.getHeaderGroupProps()}>
            {headerGroup.headers.map((col, index) => {
              return <Table.HeadCell {...col.getHeaderProps()}>{col.render("Header", { index })}</Table.HeadCell>;
            })}
          </Table.HeadRow>
        ))}
      </Table.Head>
        ...
        {rows.map((row, index) => {
          ...
          return (
            <Table.Row {...row.getRowProps()} className='px-base' key={index}>
              {row.cells.map((cell) => {
                return (
                  <Table.Cell width={cell.column.width} {...cell.getCellProps()}>
                    {cell.render("Cell")}
                  </Table.Cell>
                );
              })}
