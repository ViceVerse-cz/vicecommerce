...
const KeyboardShortcuts = ({ ...props }) => {
  return (
    <p {...props}>
      ...
      <span className='rounded px-1.5 py-0.5 bg-grey-10 font-semibold leading-small font-small'>K</span>
      pro vyhledávání
    </p>
  );
};
...
const OSCommandIcon = () => {
  const isMac = typeof window !== "undefined" && navigator?.platform?.toUpperCase().indexOf("MAC") >= 0 ? true : false;
  return <>{isMac ? "⌘" : "Ctrl"}</>;
};
