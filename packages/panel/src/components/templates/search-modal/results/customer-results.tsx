...
const CustomerResults = ({ customers = [], getLIProps, offset, selected }: CustomerResultsProps) => {
  return customers.length > 0 ? (
    <SectionCollapsible title={"Zákazníci"} length={customers?.length || 0}>
        ...
        <div className='flex flex-col'>
          {customers?.map((customer, index) => (
            <li
              ...
              className={clsx("px-base group py-1.5 focus:bg-grey-5 rounded-rounded", {
