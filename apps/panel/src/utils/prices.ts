import { LineItem, LineItemTaxLine, MoneyAmount, Order, Region } from '@medusajs/medusa';
...
export function displayAmount(currency: string, amount: number) {
  ...
  return normalizedAmount.toFixed(currencies[currency.toUpperCase()].decimal_digits);
}

export const extractUnitPrice = (item: PricedVariant, region: Region, withTax = true) => {
  let itemPrice = item.original_price;
  let includesTax = item.original_price_includes_tax;
  let taxRate = item.original_tax;

  if (itemPrice === undefined) {
    const regionPrice = item.prices.find((p) => p.currency_code === region.currency_code);
