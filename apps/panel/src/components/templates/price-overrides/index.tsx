...
const PriceOverrides = ({
  ...
  React.useEffect(() => {
    if (prices.length > 0 && variants?.length > 0) {
      const selectedVariantId = defaultVariant ? defaultVariant.id : prices[0]?.variant_id;
        ...
        {mode === MODES.SELECTED_ONLY && !isEdit && (
          <div className='pt-6 flex flex-col gap-2'>
            {variants.map((variant, idx) => (
              <div id={variant.id} className='py-2.5 px-3 border border-grey-20 rounded-rounded'>
                <ControlledCheckbox
                  control={control}
                  name='variants'
...
const ControlledCheckbox = ({ control, name, id, index, value, ...props }: ControlledCheckboxProps) => {
