...
const OrderResults = ({ orders = [], getLIProps, offset, selected }: OrderResultsProps) => {
  return orders.length > 0 ? (
    <SectionCollapsible title={"Objednávky"} length={orders?.length || 0}>
            ...
            >
              ...
              >
                <div className='flex items-center gap-x-3'>
                  <span className='inter-small-semibold'>#{order.display_id}</span>
                  <p className='inter-small-regular text-grey-90'>{order.email}</p>
                </div>
                <span
