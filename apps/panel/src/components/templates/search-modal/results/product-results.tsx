...
const ProductResults = ({ products = [], getLIProps, offset, selected }: ProductResultsProps) => {
        ...
        <div className='flex flex-col'>
          {products?.slice(0, 5).map((product, index) => (
            <li
              ...
              className={clsx('px-base py-1.5 group focus:bg-grey-5 rounded-rounded', {
                'bg-grey-5': selected === offset + index,
              })}
            >
              <Link to={`/a/products/${product.id}`} className='flex items-center justify-between'>
                <div className='flex items-center gap-x-3'>
                  <img src={product.thumbnail} className='h-[32px] w-[24px] object-cover rounded' />
                  <p className='inter-small-regular text-grey-90'>{product.title}</p>
                </div>
                <span
                  className={clsx('group-focus:visible text-grey-40 inter-small-regular', {
