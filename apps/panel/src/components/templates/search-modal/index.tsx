import { useAdminCustomers, useAdminDiscounts, useAdminOrders, useAdminProducts } from "medusa-react";
...
const SearchModal = ({ handleClose }) => {
  ...
  const isFetching = isFetchingDiscounts || isFetchingCustomers || isFetchingProducts || isFetchingOrders;
  ...
  return (
    <RadixDialog.Root open onOpenChange={handleClose}>
      <RadixDialog.Portal>
        <RadixDialog.Overlay
          className={clsx("fixed pt-[140px] pb-[100px] z-50 inset-0 backdrop-blur-sm", {
            flex: totalLength > 0,
          })}
        >
          <RadixDialog.Content
            className={clsx("max-w-[640px] flex-1 flex mx-auto bg-grey-0 rounded-rounded shadow-searchModal")}
          >
            <div className='py-large flex-1 flex flex-col'>
              <div className='flex items-center gap-x-4 pb-large border-solid px-xlarge border-b border-grey-20'>
                ...
                <Input
                  ...
                  {...getInputProps()}
                />
                <Tooltip className='bg-grey-0' onClick={handleClear} content='Vymazat vyhledávání'>
              ...
              {totalLength > 0 ? (
                <ul {...getULProps()} className='flex-1 overflow-y-auto mt-large px-xlarge'>
                  {isFetching ? (
                    ...
                    <>
                      <div>
                        <OrderResults orders={orders} offset={0} getLIProps={getLIProps} selected={selected} />
                      </div>
