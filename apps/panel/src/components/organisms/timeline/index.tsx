...
const Timeline: React.FC<TimelineProps> = ({ orderId }) => {
  ...
  const actions: ActionType[] = [
    {
      ...
      label: "Žádost o vrácení",
      onClick: () => setShowRequestReturn(true),
    },
    {
      icon: <RefreshIcon size={20} />,
      label: "Registrovat výměnu",
      onClick: () => setshowCreateSwap(true),
    },
    {
      icon: <AlertIcon size={20} />,
      label: "Zaregistrovat nárok",
  ...
  const handleCreateNote = (value: string | undefined) => {
    ...
    createNote.mutate(
      ...
      {
        onSuccess: () => notification("Úspěch", "Přidána poznámka", "success"),
  ...
  return (
    <>
      <div className='h-full w-5/12 rounded-rounded bg-grey-0 border border-grey-20'>
        <div className='py-large px-xlarge border-b border-grey-20'>
          <div className='flex items-center justify-between'>
            <h3 className='inter-xlarge-semibold'>Časová osa</h3>
            <div
              className={clsx({
      ...
      {showRequestReturn && order && <ReturnMenu order={order} onDismiss={() => setShowRequestReturn(false)} />}
      {showCreateSwap && order && <SwapMenu order={order} onDismiss={() => setshowCreateSwap(false)} />}
      {showCreateClaim && order && <ClaimMenu order={order} onDismiss={() => setshowCreateClaim(false)} />}
    </>
  );
};
