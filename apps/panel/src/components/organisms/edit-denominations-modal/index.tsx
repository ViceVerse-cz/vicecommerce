...
const EditDenominationsModal = ({
  ...
  const [denominations, setDenominations] = React.useState(augmentWithIds(defaultDenominations));
  const selectedCurrencies = denominations.map((denomination) => denomination.currency_code);
  const availableCurrencies = currencyCodes?.filter((currency) => !selectedCurrencies.includes(currency));
        ...
        <Modal.Content>
          <div className='pt-1'>
            ...
            {denominations.map((field, index) => {
              return (
                <div key={field.indexId} className='first:mt-0 mt-xsmall flex items-center'>
                  ...
                  <button className='ml-2xlarge'>
                    <TrashIcon onClick={onClickDelete(index)} className='text-grey-40' size='20' />
                  </button>
                </div>
              );
            })}
