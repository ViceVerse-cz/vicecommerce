...
const InviteModal: React.FC<InviteModalProps> = ({ handleClose }) => {
  ...
  const onSubmit = (data: InviteModalFormData) => {
    mutate(
      ...
      {
        onSuccess: () => {
          notification('Úspěch', `Pozvánka odeslána uživateli ${data.user}`, 'success');
          handleClose();
        },
        onError: (error) => {
          ...
          <Modal.Content>
            <div className='flex flex-col gap-y-base'>
              ...
              <Controller
                name='role'
                control={control}
                defaultValue={{ label: 'Člen', value: 'member' }}
                render={({ field: { value, onChange } }) => {
                  return <Select label='Role' onChange={onChange} options={roleOptions} value={value} />;
                }}
              />
            </div>
          </Modal.Content>
