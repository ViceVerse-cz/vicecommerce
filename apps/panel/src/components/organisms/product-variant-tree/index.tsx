...
const ProductVariantTree: React.FC<ProductVariantTreeProps> = ({
  ...
  return (
    <CollapsibleTree>
      <CollapsibleTree.Parent actions={getProductActions && getProductActions(product as Product)}>
        <div>
      ...
      <CollapsibleTree.Content>
        {product.variants.map((variant) => (
          <CollapsibleTree.Leaf
            key={variant.id}
            actions={getVariantActions && getVariantActions(variant as ProductVariant)}
          >
