...
type DotProps = RadioGroupPrimitive.RadioGroupItemProps & React.RefAttributes<HTMLButtonElement>;
      ...
      {!disabled ? (
        ...
        >
          ...
          <RadioGroupPrimitive.Indicator
            ...
            aria-hidden='true'
            className={clsx('absolute inset-0 shadow-violet-60 shadow-[0_0_0_2px] rounded-rounded')}
          />
        </RadioGroupPrimitive.Item>
      ) : (
      ...
      <div className='truncate'>
        <div className='flex items-center'>
          <p className='inter-base-semibold truncate'>
            {label} {sublabel ? <span className='inter-base-regular text-grey-50'>{sublabel}</span> : null}
          </p>
        </div>
        {description && (
          <p
            className={clsx('inter-small-regular text-grey-50 mt-2xsmall truncate', {
...
const SimpleItem: React.FC<RadioGroupSimpleItemProps> = ({ label, description, className, ...rest }) => {
  return (
    <label
      className={clsx(
    ...
    >
      ...
      <div className='ml-small inter-base-regular cursor-pointer w-full'>
        <span className='ml-small inter-base-regular cursor-pointer'>{label && label}</span>
        <span className='ml-small inter-base-regular cursor-pointer'>{description && description}</span>
      </div>
    </label>
  );
};
