...
const BatchJobFileCard = ({ fileName, fileSize, icon, onClick }: Props) => {
  ...
  return (
    <div className='flex items-center w-full cursor-pointer mt-4' onClick={preparedOnClick}>
      <div
      ...
      <div className='text-left relative w-full pl-4'>
        <div className='overflow-hidden truncate inter-small-regular max-w-[80%]'>{fileName}</div>

        {!!fileSize && <div className='text-grey-40 inter-small-regular'>{fileSize}</div>}
      </div>
    </div>
  );
};
