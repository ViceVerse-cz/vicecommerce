...
export const AdjacentContainer = forwardRef<HTMLDivElement, AdjacentContainerProps>(
  ({ label, helperText, required, errors, name, children }: AdjacentContainerProps, ref) => {
    return (
  ...
  return (
    <div
      ...
      className={cx(
        ...
        clsx('relative pointer-events-auto', { 'text-grey-40': isDisabled }, className),
      )}
    >
      {children}
    </div>
...
export const IndicatorsContainer = <Option, IsMulti extends boolean, Group extends GroupBase<Option>>({
  className,
  cx,
  innerProps,
  children,
