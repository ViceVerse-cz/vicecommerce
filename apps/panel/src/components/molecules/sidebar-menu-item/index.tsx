...
const SidebarMenuItem: React.FC<SidebarMenuItemProps> & {
...
} = ({ pageLink, icon, text, triggerHandler, subItems = [] }: SidebarMenuItemProps) => {
  const styles = 'group py-1.5 my-0.5 rounded-rounded flex text-grey-50 hover:bg-grey-10 items-center px-2';
  const activeStyles = 'bg-grey-10 is-active';
  const classNameFn = useCallback(({ isActive }) => (isActive ? `${styles} ${activeStyles}` : styles), []);
  ...
  return (
    ...
    >
      {subItems?.length > 0 &&
        subItems.map(({ pageLink, text }) => <SubItem pageLink={pageLink} text={text} />)}
    </Collapsible>
  );
};
