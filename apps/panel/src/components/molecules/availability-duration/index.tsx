...
const getValue = (e: React.ChangeEvent<HTMLInputElement>) => parseFloat(e.target.value);

const AvailabilityDuration: React.FC<AvailabilityDurationProps> = ({ value, onChange }) => {
  ...
  useEffect(() => {
    const isoString = `P${durationYears || 0}Y${durationMonths || 0}M${durationDays || 0}DT${
      durationHours || 0
    }H${durationMinutes || 0}M`;

    onChange(isoString);
  }, [durationYears, durationMonths, durationDays, durationHours, durationMinutes]);
