  ...
  const args = {
    ...
    children: (
      <div className='flex flex-col w-full gap-y-xsmall'>
        {event.reason && (
          <span className='text-grey-50'>{`${event.reason.slice(0, 1).toUpperCase()}${event.reason.slice(
            1,
          )}`}</span>
        )}
        {event.note && <div className='rounded-2xl px-base py-base bg-grey-5'>Note: {event.note}</div>}
      </div>
    ),
  };
