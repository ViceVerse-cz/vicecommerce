...
const Claim: React.FC<ClaimProps> = ({ event, refetch }) => {
  ...
  return (
    <>
      <EventContainer {...args} />
      {showCancel && (
        <DeletePrompt handleClose={() => setShowCancel(false)} onDelete={async () => handleCancel()} />
      )}
      {showDetails && (
        <ClaimDetails onDismiss={() => setShowDetails(false)} claim={event.claim} order={event.order} />
      )}
      {showFulfillment && (
  ...
  return event.claimType === 'replace' ? (
    ...
    <div className='flex flex-col'>
      <span className='text-grey-50 mb-2xsmall'>{`${
        event.refundStatus && event.refundStatus === 'refunded' ? 'Refunded' : 'Refund'
      } amount:`}</span>
