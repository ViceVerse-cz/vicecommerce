...
const Claim: React.FC<ClaimProps> = ({ event, refetch }) => {
  ...
  const args = {
    ...
    title: event.canceledAt ? "Reklamace zrušena" : "Vytvořený reklamační požadavek",
    topNode: actions,
    children: [
  ...
  return (
    <>
      <EventContainer {...args} />
      {showCancel && <DeletePrompt handleClose={() => setShowCancel(false)} onDelete={async () => handleCancel()} />}
      {showDetails && <ClaimDetails onDismiss={() => setShowDetails(false)} claim={event.claim} order={event.order} />}
      {showFulfillment && (
...
function ClaimItems(event: ClaimEvent) {
  return (
    <div className='flex flex-col gap-y-small'>
      <span className='inter-small-regular text-grey-50'>Reklamované položky</span>
      <div>
...
function ClaimStatus(event: ClaimEvent) {
  return event.claimType === "refund" && event.refundStatus ? (
    <div>
      <span className='text-grey-50 mb-2xsmall'>Vrácení peněz:</span>
      <RefundStatus refundStatus={event.refundStatus} />
    </div>
  ) : event.claimType === "return" && event.fulfillmentStatus ? (
    <div>
      <span className='text-grey-50 mb-2xsmall'>Doprava:</span>
  ...
  return event.claimType === "replace" ? (
    ...
    <div className='flex flex-col'>
      <span className='text-grey-50 mb-2xsmall'>{`${
        event.refundStatus && event.refundStatus === "refunded" ? "Vráceno" : "Vrácení peněz"
      } amount:`}</span>
...
function ClaimActions(event: ClaimEvent, onCancel: () => void, onFulfill: () => void, onDetails: () => void) {
  ...
  if (!(event.canceledAt || event.isCanceled)) {
    if (
      ...
      (event.fulfillmentStatus === "not_fulfilled" || event.fulfillmentStatus === "canceled")
    ) {
      actions.push({
        icon: <CheckCircleIcon size={20} />,
        label: "Splnit reklamaci",
        onClick: onFulfill,
      });
    }
