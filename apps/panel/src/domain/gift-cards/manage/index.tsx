...
import { GiftCardFormProvider, useGiftCardForm } from './form/gift-card-form-context';
...
const ManageGiftCard = () => {
  ...
  return (
    <GiftCardFormProvider giftCard={giftCardToFormValuesMapper(giftCard)} giftCardId={giftCard.id}>
...
const UpdateNotification = ({ isLoading = false }) => {
  ...
  const isDirty = checkForDirtyState(formState.dirtyFields, additionalDirtyState);
  ...
  return (
    <Toaster visible={visible} duration={Infinity} id={TOAST_ID} position='bottom-right'>
      <FormToasterContainer isLoading={isLoading}>
        <FormToasterContainer.Actions>
          <FormToasterContainer.ActionButton onClick={onUpdate}>Save</FormToasterContainer.ActionButton>
          <FormToasterContainer.DiscardButton onClick={resetForm}>Discard</FormToasterContainer.DiscardButton>
        </FormToasterContainer.Actions>
