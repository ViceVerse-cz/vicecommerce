...
const Billing = () => {
  ...
  return (
    <div className='min-h-[705px]'>
      <span className='inter-base-semibold'>Fakturační adresa</span>
      {!isNullishObject(shippingAddress) || shippingAddressId ? (
        <div className='items-center flex mt-4 mb-6 cursor-pointer' onClick={updateSameAsShipping}>
          <div
            className={`w-5 h-5 flex justify-center text-grey-0 border-grey-30 border rounded-base ${
              sameAsShipping && "bg-violet-60"
            }`}
          >
            <span className='self-center'>{sameAsShipping && <CheckIcon size={16} />}</span>
          </div>
          <input className='hidden' type='checkbox' {...form.register("same_as_shipping")} tabIndex={-1} />
          <span className='ml-3 text-grey-90'>Použijte stejný jako při přepravě</span>
        </div>
      ) : null}
      <div
