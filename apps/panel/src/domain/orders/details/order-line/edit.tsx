...
const OrderEditLine = ({ item, currencyCode, change, customerId, regionId }: OrderEditLineProps) => {
  ...
  const { mutateAsync: removeItem } = useAdminOrderEditDeleteLineItem(item.order_edit_id!, item.id);
