...
const SwapMenu: React.FC<SwapMenuProps> = ({ order, onDismiss }) => {
  ...
  const { shipping_options: shippingOptions, isLoading: shippingLoading } = useAdminShippingOptions({
    is_return: true,
    region_id: order.region_id,
  });

  const returnTotal = useMemo(() => {
    const items = Object.keys(toReturn).map((t) => allItems.find((i) => i.id === t));
  ...
  const additionalTotal = useMemo(() => {
    return itemsToAdd.reduce((acc, next) => {
      let amount = next.prices.find((ma) => ma.region_id === order.region_id)?.amount;

      if (!amount) {
        amount = next.prices.find((ma) => ma.currency_code === order.currency_code)?.amount;
      }
      ...
      <Modal.Body>
        ...
        <Modal.Footer>
          <div className='flex w-full justify-between'>
            ...
            <Button
              onClick={onSubmit}
              disabled={Object.keys(toReturn).length === 0 || itemsToAdd.length === 0}
              loading={isLoading}
              type='submit'
              variant='primary'
            >
