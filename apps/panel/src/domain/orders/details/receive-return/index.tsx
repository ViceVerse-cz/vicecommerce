...
export const ReceiveReturnMenu = ({ order, returnRequest, onClose }: Props) => {
  ...
  const onSubmit = handleSubmit((data) => {
    ...
    mutate(
      ...
      {
        onSuccess: () => {
          notification("Successfully received return", `Received return for order #${order.display_id}`, "success");
        ...
        onError: (error) => {
          notification("Failed to receive return", getErrorMessage(error), "error");
        },
      },
    );
  });
        ...
        <form onSubmit={onSubmit}>
          ...
          <Modal.Footer>
            <div className="flex w-full items-center justify-end gap-x-xsmall">
              ...
              <Button size="small" variant="primary" disabled={!isDirty || isLoading} loading={isLoading}>
                Save and close
              </Button>
            </div>
          </Modal.Footer>
