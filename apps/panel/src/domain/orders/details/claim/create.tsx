...
const ClaimMenu: React.FC<ClaimMenuProps> = ({ order, onDismiss }) => {
  ...
  const [shippingAddress, setShippingAddress] = useState<AddressPayload>(undefined);
  ...
  const [returnShippingMethod, setReturnShippingMethod] = useState<ShippingOption | null>(null);
  const [returnShippingPrice, setReturnShippingPrice] = useState<number | undefined>(undefined);
  ...
  useEffect(() => {
    if (toReturn) {
      if (Object.keys(toReturn).length !== 0 && isReplace && itemsToAdd.length > 0 && shippingMethod) {
        setReady(true);
          ...
          <div className='mt-4'>
            <h3 className='inter-base-semibold'>
              ...
              {returnShippingMethod && (
                <span className='text-grey-40 inter-base-regular'>({returnShippingMethod.region.name})</span>
              )}
            </h3>
            <Select
              clearSelected
                  ...
                  <Button
                    ...
                    onClick={() => {
                      layeredModalContext.push(
                        SelectProductsScreen(layeredModalContext.pop, itemsToAdd, handleProductSelect),
                      );
                    }}
                  >
                    Add Product
