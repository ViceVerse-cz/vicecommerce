...
const ReturnMenu: React.FC<ReturnMenuProps> = ({ order, onDismiss }) => {
  ...
  const [toReturn, setToReturn] = useState<Record<string, { quantity: number }>>({});
  ...
  const { isLoading: shippingLoading, shipping_options: shippingOptions } = useAdminShippingOptions({
