  ...
  const columns = useMemo(
    () => [
      columnHelper.display({
        ...
        cell: ({ row: { getIsSelected, getIsSomeSelected, getToggleSelectedHandler, getCanSelect } }) => {
      ...
      columnHelper.accessor("title", {
        header: ({ header: { column: { getToggleSortingHandler, getIsSorted } } }) => (
          // rome-ignore lint/a11y/useKeyWithClickEvents: <explanation>
          <div className="flex cursor-pointer items-center" onClick={getToggleSortingHandler()}>
            <p className="select-none">Title</p>
            {/* rome-ignore lint/a11y/useKeyWithClickEvents: <explanation> */}
            <div className="flex h-large w-large items-center justify-center" onClick={getToggleSortingHandler()}>
              <SortingIcon size={16} isSorted={getIsSorted()} />
            </div>
          </div>
        ),
        cell: ({ cell: { getValue }, row: { original: { product, sku }, getCanSelect } }) => {
          const isSelectable = getCanSelect();

          return (
            <div
              className={clsx("inter-small-regular flex items-center gap-base py-xsmall", {
            ...
            >
              ...
              <div>
                <p>
                  {product?.title} <span className="text-grey-50">({getValue()})</span>
                </p>
      ...
      columnHelper.accessor("calculated_price_incl_tax", {
        ...
        cell: ({ getValue, row: { original: { original_price_incl_tax } } }) => {
      ...
      columnHelper.display({
        ...
        maxSize: 24,
        cell: () => {
          return <p className="pl-base text-grey-50">{order.currency_code.toUpperCase()}</p>;
        },
      }),
    ],
    [order],
