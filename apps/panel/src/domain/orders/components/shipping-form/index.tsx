...
const ShippingForm = ({ form, order, isReturn = false, isClaim = false, required = false }: Props) => {
  const {
    control,
    path,
    setValue,
    ...
    return (
      shippingOptions?.map((o) => ({
        ...
        value: {
          id: o.id,
          taxRate:
            (o as unknown as PricedShippingOption).tax_rates?.reduce((acc, cur) => acc + (cur.rate || 0) / 100, 0) || 0,
        },
        suffix: (
          <span
            className={clsx({
          ...
          >
            {formatAmountWithSymbol({
              amount: (o as unknown as PricedShippingOption).price_incl_tax || o.amount || 0,
  ...
  const setCustomPrice = () => {
    if (selectedOption) {
      const option = shippingOptions?.find((ro) => ro.id === selectedOption.value.id);

      setValue(path("price"), Math.round((option?.amount || 0) * (1 + selectedOption.value.taxRate)));
    }
  };
  ...
  return (
    <div className="flex flex-col gap-y-base">
      <div className="flex flex-col">
        <h2 className="inter-base-semibold">Shipping for {isReturn ? "return" : "replacement"} items</h2>
      ...
      {selectedOption && !isClaim && (
        <div className="flex w-full items-center justify-end">
          {selectedOptionPrice !== undefined ? (
            ...
            <Button size="small" variant="secondary" type="button" className="h-10" onClick={setCustomPrice}>
              Add custom price
            </Button>
          )}
        </div>
...
const ShippingFormHelpText = ({ isClaim = false, isReturn = false }: Pick<Props, "isClaim" | "isReturn">) => {
  const text = useMemo(() => {
    if (isClaim && isReturn) {
