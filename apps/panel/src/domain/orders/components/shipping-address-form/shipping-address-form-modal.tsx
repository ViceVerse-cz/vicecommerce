...
const ShippingAddressFormScreen = ({ form, order }: Props) => {
  ...
  return (
    <>
      <Modal.Content>
        <AddressForm form={form} type={AddressType.SHIPPING} countryOptions={countryOptions} required={true} />
      </Modal.Content>
      <Modal.Footer>
        <div className="flex w-full items-center justify-end gap-x-xsmall">
          <Button variant="secondary" size="small" type="button" onClick={cancelAndPop}>
            Cancel
          </Button>
          <Button variant="primary" size="small" disabled={!isValid} type="button" onClick={pop}>
            Save and go back
          </Button>
        </div>
      </Modal.Footer>
