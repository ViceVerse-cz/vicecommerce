...
export const ReceiveReturnSummary = ({ form, order, returnRequest }: Props) => {
  ...
  const shipping = useMemo(() => {
    ...
    const taxRate = returnRequest.shipping_method.tax_lines.reduce((acc, tax) => {
     ...
     */
    const priceInclTax = Math.round(returnRequest.shipping_method.price * (1 + taxRate));
  ...
  return (
    <div className="inter-base-regular">
      ...
      <div className="flex flex-col gap-y-xsmall pt-large">
        <div className="inter-large-semibold flex items-center justify-between" data-testid="refund-amount-container">
