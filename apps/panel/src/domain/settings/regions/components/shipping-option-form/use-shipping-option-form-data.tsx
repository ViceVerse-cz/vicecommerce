...
export const useShippingOptionFormData = (regionId: string, isReturn = false) => {
    ...
    const options = fulfillment_options.reduce((acc, current, index) => {
      ...
      return acc.concat(
        filtered.map((option, o) => ({
          label: `${option.name || option.id} via ${fulfillmentProvidersMapper(current.provider_id).label}`,
          value: `${index}.${o}`,
        })),
      );
    }, [] as Option[]);
