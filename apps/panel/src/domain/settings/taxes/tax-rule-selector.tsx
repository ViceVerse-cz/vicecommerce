...
const TaxRuleSelector: React.FC<TaxRuleSelectorProps> = ({ regionId, type, items, onSubmit }) => {
  const isLocked = type && items;

  const { pop } = useContext(LayeredModalContext);
  const [selectedType, setSelectedType] = useState<string>(type ?? TaxRuleType.PRODUCTS);
      ...
      <Modal.Content>
        <div className='min-h-[680px]'>
          ...
          {selectedType === TaxRuleType.PRODUCTS && (
            <ProductSelector items={selectedRule.items} onChange={handleItemChanges} />
          )}
