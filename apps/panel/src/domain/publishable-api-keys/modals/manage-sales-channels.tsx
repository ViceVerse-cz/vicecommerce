...
function AddScreen(props: { keyId: string; close: () => void; goBack: () => void; isVisible: boolean }) {
 ...
 */
function EditScreen(props: { keyId: string; close: () => void; goAdd: () => void; isVisible: boolean }) {
  const { close } = props;
  ...
  // virtual pagination
  const displayData = useMemo(() => data?.slice(offset, offset + LIMIT), [data, offset]);
      ...
      <div className='flex-grow'>
        <div className='flex justify-between items-center gap-2 my-6'>
          ...
          {selectedCount ? (
            ...
            <Button size='small' className='flex-shrink-0 h-[32px]' variant='secondary' onClick={props.goAdd}>
              Add channels
            </Button>
          )}
        </div>
 ...
 */
function ManageSalesChannelsSideModal(props: ManageSalesChannelsSideModalProps) {
