...
const PriceFormInput = ({ name, currencyCode, errors, amount, onChange }: Props) => {
  const { symbol_native, decimal_digits } = currencies[currencyCode.toUpperCase()];
  ...
  const [formattedValue, setFormattedValue] = useState<string | undefined>(
    amount !== null && amount !== undefined ? getFormattedValue(amount) : undefined,
  );
    ...
    <div>
      ...
      >
        ...
        <AmountField
          ...
          onValueChange={(value, _name, values) => onAmountChange(value, values?.float)}
