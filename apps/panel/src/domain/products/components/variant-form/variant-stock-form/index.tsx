...
const VariantStockForm = ({ form }: Props) => {
  ...
  return (
    <div>
      <p className='inter-base-regular text-grey-50'>Configure the inventory and stock for this variant.</p>
      <div className='pt-large flex flex-col gap-y-xlarge'>
        ...
        <div className='grid grid-cols-2 gap-large'>
          ...
          />
          <InputField label='EAN (Barcode)' placeholder='123456789102...' {...register(path('ean'))} />
          <InputField label='UPC (Barcode)' placeholder='023456789104...' {...register(path('upc'))} />
          <InputField label='Barcode' placeholder='123456789104...' {...register(path('barcode'))} />
        </div>
      </div>
    </div>
  );
