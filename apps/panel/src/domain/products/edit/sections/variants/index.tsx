...
const VariantsSection = ({ product }: Props) => {
  ...
  const { state: addVariantState, close: closeAddVariant, toggle: toggleAddVariant } = useToggleState();

  const { state: editVariantsState, close: closeEditVariants, toggle: toggleEditVariants } = useToggleState();
  ...
  return (
    <OptionsProvider product={product}>
      ...
      <OptionsModal open={optionState} onClose={closeOptions} product={product} />
      <AddVariantModal open={addVariantState} onClose={closeAddVariant} product={product} />
      <EditVariantsModal open={editVariantsState} onClose={closeEditVariants} product={product} />
      {variantToEdit && (
        <EditVariantModal
            ...
            <div key={i}>
              ...
              <ul className='flex flex-wrap items-center gap-1'>
                {Array.from(Array(3)).map((_, j) => (
                  <li key={j}>
                    <div className='text-grey-50 bg-grey-10 h-8 w-12 animate-pulse rounded-rounded'>{j}</div>
                  </li>
                ))}
              </ul>
            </div>
