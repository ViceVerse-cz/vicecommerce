...
export const VariantCard = ({ id, index, ean, sku, title, moveCard, product }: VariantCardProps) => {
  const {
    register,
    formState: { errors },
  } = useFormContext();
  ...
  return (
    ...
    >
      <div className='text-grey-40 cursor-move flex items-center justify-center' ref={ref}>
        <GripIcon size={20} />
      </div>
      <div className='flex flex-col text-left ml-base justify-center'>
        <p className='inter-base-semibold'>
          {title}
          {sku && <span className='text-grey-50 inter-base-regular'>({sku})</span>}
        </p>
