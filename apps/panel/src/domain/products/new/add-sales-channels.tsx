...
const AddSalesChannelsForm = ({ form }: Props) => {
  ...
  const [removedDefaultSalesChannel, setRemovedDefaultSalesChannel] = useState(false);
  ...
  const { state: open, toggle: toggleModal, close: closeModal } = useToggleState();
  ...
  useEffect(() => {
    if (store?.default_sales_channel && fields) {
      const alreadyAdded = fields.find(({ id }) => id === store.default_sales_channel.id);
       ...
       */
      const duplicates = fields.filter((channel, index) => fields.findIndex((c) => c.id === channel.id) !== index);
      ...
      <div>
        <div>
          <div className='flex items-center justify-between'>
            <h2 className='inter-base-semibold'>Prodejní kanály</h2>
            <Switch checked={isEnabled} onCheckedChange={toggleEnabled} />
          </div>
          <p className='inter-base-regular text-grey-50 mt-2xsmall'>
            Tento produkt bude k dispozici pouze ve výchozím prodejním kanálu, pokud nebude změněn.
          </p>
        </div>
        <div
          className={clsx({
        ...
        >
          ...
          <Button variant='secondary' className='w-full h-[40px] mt-large' type='button' onClick={toggleModal}>
            <ChannelsIcon size={20} />
            <span>Změna dostupnosti</span>
          </Button>
        </div>
      </div>
      <SalesChannelsModal source={fields} open={open} onClose={closeModal} onSave={onAddChannels} />
    </>
  );
};
