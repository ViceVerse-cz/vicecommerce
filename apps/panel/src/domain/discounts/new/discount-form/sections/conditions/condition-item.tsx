...
const ConditionItem = <Type extends DiscountConditionType>({
  ...
  const { discount_condition } = useAdminGetDiscountCondition(discountId!, conditionId!, queryParams, {
  ...
  return (
    <div>
      ...
      {showEdit && <EditConditionsModal onClose={() => setShowEdit(false)} view={type} />}
    </div>
  );
};
