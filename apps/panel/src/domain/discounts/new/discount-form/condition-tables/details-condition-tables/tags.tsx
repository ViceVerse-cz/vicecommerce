...
const DetailsTagConditionSelector = ({ onClose }: AddConditionSelectorProps) => {
  ...
  const [operator, setOperator] = useState<DiscountConditionOperator>(conditions.product_tags.operator);

  const { isLoading, count, product_tags } = useAdminProductTags(params.queryObject, {
    keepPreviousData: true,
  });

  const changed = (values: string[]) => {
    const selectedTags = product_tags?.filter((t) => values.includes(t.id)) || [];
  ...
  return (
    <>
      ...
      <Modal.Footer>
        <DetailsConditionFooter type='product_tags' items={items} onClose={onClose} operator={operator} />
      </Modal.Footer>
    </>
  );
};
