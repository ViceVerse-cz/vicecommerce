...
const DetailsTypeConditionSelector = ({ onClose }: AddConditionSelectorProps) => {
  ...
  const [operator, setOperator] = useState<DiscountConditionOperator>(conditions.product_types.operator);

  const { isLoading, count, product_types } = useAdminProductTypes(params.queryObject, {
    // avoid UI flickering by keeping previous data
    keepPreviousData: true,
  });

  const changed = (values: string[]) => {
    const selectedTypes = product_types?.filter((type) => values.includes(type.id)) || [];
  ...
  return (
    <>
      ...
      <Modal.Footer>
        <DetailsConditionFooter type='product_types' items={items} onClose={onClose} operator={operator} />
      </Modal.Footer>
    </>
  );
};
