...
const PricesTable = ({ id, selectProduct }: PricesTableProps) => {
  ...
  const { products, isLoading, count = 0 } = useAdminPriceListProducts(id, params.queryObject);
...
const ProductHeader = ({ headerGroup }: { headerGroup: HeaderGroup<Product> }) => {
  return (
...
const PricesTableRow = ({ children, priceListId, product, onClick, ...props }) => {
  const notification = useNotification();
  const deleteProductPrices = useAdminDeletePriceListProductPrices(priceListId, product.id);
