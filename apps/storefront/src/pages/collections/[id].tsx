...
const CollectionPage: NextPageWithLayout<PrefetchedPageProps> = ({ notFound }) => {
  const { query, isFallback, replace } = useRouter();
  const id = typeof query.id === "string" ? query.id : "";

  const { data, isError, isSuccess, isLoading } = useQuery(["get_collection", id], () => fetchCollection(id));
...
export const getStaticProps: GetStaticProps = async (context) => {
  ...
  await queryClient.prefetchQuery(["get_collection", id], () => fetchCollection(id));
