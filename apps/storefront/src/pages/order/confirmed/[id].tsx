...
const Confirmed: NextPageWithLayout = () => {
  ...
  const { isSuccess, data, isLoading, isError } = useQuery(["get_order_confirmed", id], () => fetchOrder(id), {
    enabled: id.length > 0,
    staleTime: Infinity,
  });
  ...
  if (isSuccess) {
    return (
      <>
        <Head title="Order Confirmed" description="You purchase was successful" />
...
export const getStaticProps: GetStaticProps = async (context) => {
  ...
  await queryClient.prefetchQuery(["get_order_confirmed", id], () => fetchOrder(id));
