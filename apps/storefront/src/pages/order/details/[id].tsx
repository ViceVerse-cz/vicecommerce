...
const Confirmed: NextPageWithLayout = () => {
  ...
  const { isSuccess, data, isLoading, isError } = useQuery(["get_order_details", id], () => fetchOrder(id), {
    enabled: id.length > 0,
  ...
  if (isSuccess) {
    return (
      <>
        <Head title={`Order #${data.display_id}`} description="View your order" />
...
export const getStaticProps: GetStaticProps = async (context) => {
  ...
  await queryClient.prefetchQuery(["get_order_details", id], () => fetchOrder(id));
