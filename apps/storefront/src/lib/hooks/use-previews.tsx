...
const usePreviews = <T extends InfiniteProductPage>({ pages, region }: UsePreviewProps<T>) => {
  const previews: ProductPreviewType[] = useMemo(() => {
    ...
    const transformedProducts = products.map((p) => transformProductPreview(p, region));
