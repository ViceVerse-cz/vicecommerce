...
const NativeSelect = forwardRef<HTMLSelectElement, NativeSelectProps>(
  ({ placeholder = "Select...", errors, touched, className, children, ...props }, ref) => {
    const innerRef = useRef<HTMLSelectElement>(null);
    const [isPlaceholder, setIsPlaceholder] = useState(false);

    useImperativeHandle<HTMLSelectElement | null, HTMLSelectElement | null>(ref, () => innerRef.current);

    const hasError = props.name ? get(errors, props.name) && get(touched, props.name) : false;
    ...
    return (
      <div>
        <div
          ...
          className={clsx("relative flex items-center text-base-regular border border-gray-200", className, {
