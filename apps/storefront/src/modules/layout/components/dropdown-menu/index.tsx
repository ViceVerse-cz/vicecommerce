...
import { useFeaturedProductsQuery, useNavigationCollections } from "@lib/hooks/use-layout-data";
...
const DropdownMenu = () => {
  ...
  return (
    <div onMouseEnter={() => setOpen(true)} onMouseLeave={() => setOpen(false)} className="h-full">
          ...
          <>
            <Link href="/shop" passHref>
              <a className="relative flex h-full">
                <Popover.Button
                  className={clsx("relative h-full flex items-center transition-all ease-out duration-200")}
                  onClick={() => push("/store")}
                >
                  Store
                </Popover.Button>
                ...
                <div className="relative bg-white py-8">
                  <div className="flex items-start content-container">
                    <div className="flex flex-col flex-1 max-w-[30%]">
                      <h3 className="text-base-semi text-gray-900 mb-4">Collections</h3>
                      <div className="flex items-start">
                        ...
                        {loadingCollections &&
                          repeat(6).map((index) => <div key={index} className="w-12 h-4 bg-gray-100 animate-pulse" />)}
                      </div>
                    </div>
                    <div className="flex-1">
                      <div className="grid grid-cols-3 gap-4">
                        ...
                        {loadingProducts && repeat(3).map((index) => <SkeletonProductPreview key={index} />)}
                      </div>
                    </div>
                  </div>
                </div>
