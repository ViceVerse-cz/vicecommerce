...
type UpdateCustomerNameFormData = Pick<StorePostCustomersCustomerReq, "billing_address">;
...
const ProfileBillingAddress: React.FC<MyInformationProps> = ({ customer }) => {
  ...
  const { mutate: update, isLoading, isSuccess, isError, reset: clearState } = useUpdateMe();
  ...
  const [firstName, lastName, company, address1, address2, city, province, postalCode, countryCode] = useWatch({
    control,
    name: [
    ...
    >
      ...
      >
        <div className="grid grid-cols-1 gap-y-2">
          ...
          <Input label="Company" {...register("billing_address.company")} defaultValue={company} errors={errors} />
          <Input
            label="Address"
          ...
          <Input label="Province" {...register("billing_address.province")} defaultValue={province} errors={errors} />
          <NativeSelect {...register("billing_address.country_code", { required: true })} defaultValue={countryCode}>
            <option value="">-</option>
